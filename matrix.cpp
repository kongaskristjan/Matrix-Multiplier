...
void Matrix::z_order(Matrix_view ret, Const_matrix_view a, Const_matrix_view b,
...
void Matrix::local_transform4x4(Matrix_view ret, size_t x0, size_t y0)
{
	float sw0, sw1;
	sw0 = ret.get(x0 + 1, y0);
...
void Matrix::local_transform8x8(Matrix_view ret, size_t x0, size_t y0)
{
...
void Matrix::global_transform(Matrix_view ret)
{
...
void Matrix::manage_threads(Matrix & ret, const Matrix & a,
		const Matrix & b, size_t cube)
{
	double thread_coef = 8.0;
	double nblocks = thread_coef * std::thread::hardware_concurrency();

	double sq = sqrt(1.0 / nblocks);
	size_t bx = max(16ul, ((size_t) (sq * ret.rx * ret.rx / ret.ry)) / 8 * 8);
	size_t by = max(16ul, ((size_t) (sq * ret.ry * ret.ry / ret.rx)) / 8 * 8);

	for (size_t y = 0; y < ret.ry; y += by)
		for (size_t x = 0; x < ret.rx; x += bx){
			size_t dx = min(bx, ret.rx - x);
			size_t dy = min(by, ret.ry - y);
			auto job = Matrix_mul_job(
					Matrix_view(ret, x, y, dx, dy),
					Const_matrix_view(a, y, 0, dy, a.ry),
					Const_matrix_view(b, x, 0, dx, b.ry), cube);
			_g_thread_pool.push(job);
		}

	_g_thread_pool.finish();
}
...
Matrix_mul_job::Matrix_mul_job(Matrix_view _ret, Const_matrix_view _a, Const_matrix_view _b,
		size_t _cube):
	ret(_ret), a(_a), b(_b), cube(_cube)
{}


void Matrix_mul_job::execute()
{
	Matrix::z_order(ret, a, b, cube, 0, 0, 0);
	Matrix::global_transform(ret);
}
